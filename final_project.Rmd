...
UN_data<-UNdata_Export_20191209_200452883 %>% 
  ...
  mutate(country =str_replace_all(country, "United States", "United States of America"))
  
GDP_suicide_India <- tidy_suicide_rates_per100000 %>% 
  full_join(UN_data, by=c("year", "country"))%>% 
  filter(country==c("India"))

GDP_suicide_America <- tidy_suicide_rates_per100000 %>% 
  full_join(UN_data, by=c("year", "country"))%>% 
  filter(country==c("United States of America"))

GDP_suicide <- GDP_suicide_America %>% 
  full_join(GDP_suicide_India)
```

```{r}
suicide_males <- GDP_suicide %>% 
  filter(sex=="Male")

suicide_females <- GDP_suicide %>% 
  filter(sex=="Female")

suicide_both <- GDP_suicide %>% 
  filter(sex=="Both sexes")

suicide_2000 <- GDP_suicide %>% 
  filter(year=="2000")

suicide_2010 <- GDP_suicide %>% 
  filter(year== "2010")

suicide_2015 <- GDP_suicide %>% 
  filter(year== "2015")

suicide_2016 <- GDP_suicide %>% 
  filter(year== "2016")
```

```{r}
  geom_line(aes(color=country))+
  labs(x="Year", y="Suicides per 100,000 People", color= "Country", title= "Suicides per 100,000 People in USA and India")

ggplot(data= GDP_suicide, mapping= aes(x=year, y= Value, group=country))+
  geom_line(aes(color=country))+
  labs(x="Year", y="GDP per Capita", color= "Country", title= "GDP per Capita in USA and India")

ggplot(data= suicide_not_both, mapping= aes(x=year, y= rate, group=sex))+
  geom_line(aes(color=sex))+
  facet_wrap(~country)+
  labs(x="Year", y="Suicides per 100,000 People", color= "Sex", title= "Suicides per 100,000 People in USA and India by Sex")
```
